        ...
        <ul className="w-full flex flex-row gap-x-2 mb-16 justify-between ml-16">
            {props.steps.map((step, index) => (
                <li className="flex items-center gap-x-2 flex-1 group" key={`stepper-step-${index}-${step.label}`}>
                    ...
                    <span className={`size-7 flex justify-center items-center shrink-0 py-[12px] px-[17px] font-medium rounded-full ${index + 1 <= props.currentStep ? 'bg-gray-800 text-white' : 'bg-gray-300 text-gray-800'}`}>
                        {index + 1}
                    </span>
